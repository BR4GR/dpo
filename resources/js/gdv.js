...
export default class STD {
    ...
    renderStackedBarChart() {
        ...
        const svg = d3.select("#inverkehrsetzungen2")
            .append("svg")
            .attr("id", "stackedBarChartSVG") // Add an ID for easier reference
            .attr("width", "100%")
            .attr("viewBox", `0 0 ${width + margin.left + margin.right} ${height + margin.top + margin.bottom}`)
            .attr("preserveAspectRatio", "xMinYMin meet");
        ...
        // Add a button to save the chart as a PDF
        d3.select("#inverkehrsetzungen2")
            .append("button")
            .text("Save as PDF")
            .style("margin-top", "20px")
            .on("click", async () => {
                // Ensure jsPDF and svg2pdf are accessible
                const { jsPDF } = window.jspdf;

                // Select the SVG element
                const svgElement = document.getElementById("stackedBarChartSVG");
